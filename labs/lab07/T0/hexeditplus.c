...
int debug = 0;
int unitSize = 4;
char fileName[100] = "/Users/amitm/dev/archi/labs/lab07/T0/chezi";
char *menu[];
//main loc: 464. size 175
...
void convertToHex(unsigned char *arr, int size);

void convertToBigEndian(unsigned char *arr, int size);
...
void setSize() {
    do {
        ...
        fgets(reader, sizeof(reader), stdin);
        reader[strlen(reader) - 1] = 0;
        sscanf(reader, "%d", &unitSize);
    } while (unitSize != 1 && unitSize != 2 && unitSize != 4);
...
void convertToBigEndian(unsigned char *arr, int size) {
    int i;
    for (i = 0; i <= size - unitSize; i += unitSize) {
        int indexToReplace = i + unitSize - 1;
        for (int x = i; x < i + unitSize / 2; x++, indexToReplace--) {
            unsigned char temp = arr[x];
            arr[x] = arr[indexToReplace];
...
void fileModify() {
    ...
    int size = strlen(val);
    convertToHex(val, size);
    convertToBigEndian(val, size);
...
int digit_cnt(char *bla) {//1820 size 64
    int counter = 0;
    while ((*bla) != 0) {
        if ((*bla)>='0' && (*bla)<='9')
            counter++;
